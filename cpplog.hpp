...
namespace cpplog
{
	...
	public:
		...
		LogMessage(const char* file, unsigned int line, loglevel_t logLevel, BaseLogger& outputLogger, bool doInitLogMessage=true)
			: m_logger(&outputLogger)
		{
			Init(file, line, logLevel, doInitLogMessage);
		}
	...
	private:
		void Init(const char* file, unsigned int line, loglevel_t logLevel, bool doInitLogMessage=true)
		{
			...
			m_logData->threadId		= static_cast<unsigned long>(boost::interprocess::ipcdetail::get_current_thread_id());
#endif
